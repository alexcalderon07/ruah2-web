	...
	public function dataview($query, $search=NULL)
	{
		...
		if($rowCount>0)
		{
			...
			<tr>
				<th scope="row">
					<?php if($number <= 3) { ?>
						<img src="<?php print $site_url; ?>images/top-<?php print $number; ?>.png" alt="top-<?php print $number++; ?>" />
					<?php } else { ?>
						<span class="text-highlight"><?php print $number; ?>.</span>
					<?php } ?>
				</th>
	...
	{		
		...
		if($total_no_of_records > 0)
		{
			...
			for($i=$x;$i<=$x+3;$i++)
				...
				{
					if($search)
						print "<a class='current' href='".$self.$i."/".$search."' style=''>".$i."</a>&nbsp;&nbsp;";
					else
						print "<a class='current' href='".$self.$i."' style=''>".$i."</a>&nbsp;&nbsp;";
				}
