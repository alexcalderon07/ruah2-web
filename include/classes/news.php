		...
		if($rowCount>0)
		{
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
				...
				<li>
					...
					<span class="news-content"><a href="<?php print $site_url; ?>read/<?php print $row['id']; ?>"><?php print $row['title']; ?></a></span>
					<span class="news-date"><?php print $row['time']; ?></span>
				</li>
                <?php
			}
		}
	...
	{		
		...
		if($total_no_of_records > 0)
		{
			...
			for($i=$x;$i<=$x+3;$i++)
			{
				if($i==$current_page)
				{
					echo "<a class='current' href='".$self.$i."' style=''>".$i."</a>&nbsp;&nbsp;";
				}
