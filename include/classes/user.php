...
class USER
{
	...
	public function register($username,$password,$email,$ref,$promoter)
	{
		global $safebox_size;
		...
		try
		{
			...
			$stmt = $this->account->prepare("INSERT INTO account(login, password, social_id, email, create_time, status, availDt)
		                                               VALUES(:login, :password, :social_id, :email, NOW(), :status, :availDt)");
			...
			$stmt->bindparam(":availDt", $availDt);
