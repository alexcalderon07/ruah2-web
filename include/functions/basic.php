<?php
	...
	function getPlayerGuild($id)
	{
		global $database;

		$stmt = $database->runQueryPlayer("SELECT guild_id FROM guild_member WHERE pid=:id");
		$stmt->bindParam(':id', $id, PDO::PARAM_INT);
		$stmt->execute();
		$result=$stmt->fetch(PDO::FETCH_ASSOC);
		...
		$stmt = $database->runQueryPlayer("SELECT name FROM guild WHERE id=:id");
		$stmt->bindParam(':id', $result['guild_id'], PDO::PARAM_INT);
		$stmt->execute();
		$result=$stmt->fetch(PDO::FETCH_ASSOC);
	...
	function top10guilds()
	{
		...
		$stmt = $database->runQueryPlayer("SELECT name, master, level FROM guild WHERE name NOT LIKE '[%]%' ORDER BY level DESC, ladder_point DESC, exp DESC, name ASC limit 5");
		$stmt->execute();
		$top = $stmt->fetchAll();
	...
	function new_item_position_safebox($id_account, $new_item)
	{
		global $database;
			
		$sth = $database->runQueryPlayer('SELECT pos, vnum
			FROM item
			WHERE owner_id=? AND `window`="SAFEBOX" ORDER by pos ASC');
		$sth->bindParam(1, $id_account, PDO::PARAM_INT);
		$sth->execute();
		$result = $sth->fetchAll();
	...
	function insert_vote4coins($id, $ip)
	{
		...
		// First check if in account.votes there is a vote from this account_id or ip, then update the record with updated_at = now(), else, create it
		$stmt = $database->runQueryAccount("SELECT id FROM votes WHERE account_id = ?");
		$stmt->bindParam(1, $_SESSION['id'], PDO::PARAM_INT);
		$stmt->execute();
		$result = $stmt->fetchAll(PDO::FETCH_COLUMN);
		...
		if($result)
		{
			$stmt = $database->runQueryAccount("UPDATE votes SET account_ip = ?, updated_at = NOW() WHERE account_id = ?");
		...
		else
		{
			...
			$stmt = $database->runQueryAccount("INSERT INTO votes (account_id, account_name, account_ip, server, created_at, updated_at) VALUES (?, ?, ?, 1, NOW(), NOW())");
			$stmt->bindParam(1, $_SESSION['id'], PDO::PARAM_INT);
			$stmt->bindParam(2, $account_name, PDO::PARAM_STR);
			$stmt->bindParam(3, $ip, PDO::PARAM_STR);
			$stmt->execute();
		}
		
		$stmt = $database->runQueryAccount("UPDATE account SET votebuff = 1 where id = ?");
		$stmt->bindParam(1, $_SESSION['id'], PDO::PARAM_INT);
		$stmt->execute();
	...
	function getCacheFileName($key) {
		return  'cache/cache_' . md5($key) . '.txt';
	}

	function cacheIsValid($filename, $cacheTime) {
		return file_exists($filename) && (time() - filemtime($filename)) < $cacheTime;
	}

	function getStatistics($key)
	{
    $cacheTime = 10; // Cache for 5 minutes (300 seconds)
    $cacheFile = getCacheFileName($key);

    if (cacheIsValid($cacheFile, $cacheTime)) {
	...
	function fix_account_columns()
	{
		...
		foreach($fix as $column => $query)
			if(!in_array($column, $columns))
			{
				...
				print '<div class="alert alert-success" role="alert"><button type="button" class="close" data-dismiss="alert" aria-label="Close"></button><center>'.$lang['account-new-column'].$column.'</center></div>';
			}
	}
	...
	function insert_promoter($code)
	{
		global $database;

		$stmt = $database->runQuerySqlite("INSERT INTO promoters (code) VALUES (:code)");
		$stmt->execute(array(':code'=>$code));
	}

	function delete_promoter($id) {
		global $database;
		$stmt = $database->runQuerySqlite("DELETE FROM promoters WHERE id = :id");
		$stmt->execute(array(':id' => $id));
	}

	// Create a function to fetch a specific promoter by ID
	function get_promoter($id) {
		global $database;
		$stmt = $database->runQuerySqlite("SELECT * FROM promoters WHERE id = :id");
		$stmt->execute(array(':id' => $id));
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}

	function get_promoter_by_code($code) {
		global $database;
		$code = strtolower($code);
		$stmt = $database->runQuerySqlite("SELECT * FROM promoters WHERE LOWER(code) = :code");
		$stmt->execute(array(':code' => $code));
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}
	

	function get_all_promoters(){
		global $database;
		
		$stmt = $database->runQuerySqlite("SELECT * FROM promoters");
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_ASSOC);	
	}


	function update_promoter($id, $code) {
		global $database;
		$stmt = $database->runQuerySqlite("UPDATE promoters SET code = :code WHERE id = :id");
		$stmt->execute(array(':code' => $code, ':id' => $id));
	}

	function add_promoter($user_id, $promoter_id){
		//insert into referrals_promo values id, referral_id, user_id, date
		global $database;
		$stmt = $database->runQuerySqlite("INSERT INTO referrals_promo (referral_id, user_id, date) VALUES (:referral_id, :user_id, :date)");
		$stmt->execute(array(':referral_id' => $promoter_id, ':user_id' => $user_id, ':date' => date('Y-m-d H:i:s')));

	}

	function get_promoters() {
		global $database;
		// SQL query that joins the promoters table with the referrals_promo table
		// and counts the number of referrals for each promoter.
		$sql = "SELECT p.id, p.code, COUNT(rp.referral_id) AS referral_count 
				FROM promoters p
				LEFT JOIN referrals_promo rp ON p.id = rp.referral_id
