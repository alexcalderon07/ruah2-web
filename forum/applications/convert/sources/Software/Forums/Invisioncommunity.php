...
class Invisioncommunity extends Software
{
	...
	public function convertForumsRssImported()
	{
		...
		foreach( $this->fetch( 'core_rss_imported', 'rss_imported_content_id', array( 'rss_imported_id IN(' . (string) Db::i()->select( 'rss_import_id', 'core_rss_import', array( "rss_import_class='IPS\\forums\\Topic'" ) ) . ')' ) ) AS $row )
		{
			/* Remove non-standard columns */
