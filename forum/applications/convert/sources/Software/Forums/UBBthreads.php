...
use IPS\convert\Software\Core\UBBthreads;
use IPS\DateTime;
use IPS\Db;
...
class UBBthreads extends Software
{
	...
	{
		return UBBthreads::fixPostData( $post, $className, $contentId );
	}
	...
	public function convertForumsForums()
	{
		...
		foreach( $this->db->select( '*', 'FORUMS' ) AS $row )
		{
			$libraryClass->convertForumsForum( array(
				...
				'last_post'				=> DateTime::create()->setTimestamp( $row['FORUM_LAST_POST_TIME'] ),
