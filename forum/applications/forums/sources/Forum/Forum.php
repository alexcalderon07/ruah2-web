...
class Forum extends Model implements Permissions
{
	 ...
	 * @return    array|null
	 */
	public function lastPost( int $count=1, $isChild=false ): ?array
	{
		if ( $count > 5 )
		{
		...
		else
		{
			...
			if ( count( $lastPostData ) )
			{
				foreach( $lastPostData as $lastPostTime => $lastPost )
				{
					...
					$results[ $lastPostTime ] = array(
						'author'		    => null,
						'_author'			=> ! empty( $lastPost['last_poster_id'] ) ? $lastPost['last_poster_id'] : ( $lastPost['last_poster_name'] ?? '' ),
			...
			if ( count( $results ) )
			{
				...
				if ( ! $isChild )
				{
					...
					{
						$membersLoaded = iterator_to_array( Db::i()->select( '*', 'core_members', [ Db::i()->in( 'member_id', $membersToLoad ) ] )->setKeyField( 'member_id' ) );

						foreach ( $membersToLoad as $timestamp => $memberId )
						{
							if ( isset( $membersLoaded[$memberId] ) )
							{
								$results[$timestamp]['author'] = Member::constructFromData( $membersLoaded[$memberId] );
							}
						}
					}
				}
