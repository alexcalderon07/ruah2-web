 ...
 */
class MessengerConversations
{
	/*
	 * @brief 	Query description
	 */
	public static string $description = "Returns a list of messenger conversations";
		...
		return array(
			...
			'orderBy' => [
				'type' => TypeRegistry::eNum([
					...
					'values' => MessengerConversationType::getOrderByOptions()
				]),
	 ...
	 *
	 * @return ListOfType<MessengerConversationType>
	 */
	 ...
	 */
	public function resolve( mixed $val, array $args, array $context, array $info ) : ActiveRecordIterator|array
	{
		...
		if( !is_null( $args['search'] ) )
		{
			...
			if ( !is_null( $args['searchIn'] ) )
			{
				...
				{
					$query[] = Query::matchClause( "core_message_topics.mt_title", $args['search'], '+', FALSE );
				}
				if ( in_array( 'POST', $args['searchIn'] ) )
				{
					$query[] = Query::matchClause( "core_message_posts.msg_post", $args['search'], '+', FALSE );
				}
				if ( in_array( 'RECIPIENT', $args['searchIn'] ) )
				{
					$query[] = "core_message_posts.msg_topic_id IN ( SELECT sender_map.map_topic_id FROM {$prefix}core_message_topic_user_map AS sender_map WHERE sender_map.map_is_starter=1 AND sender_map.map_user_id IN ( SELECT member_id FROM {$prefix}core_members AS sm WHERE name LIKE '" . Db::i()->escape_string( $args['search'] ) . "%' ) )";
				}
				if ( in_array( 'SENDER', $args['searchIn'] ) )
				{
					$query[] = "core_message_posts.msg_topic_id IN ( SELECT receiver_map.map_topic_id FROM {$prefix}core_message_topic_user_map AS receiver_map WHERE receiver_map.map_is_starter=0 AND receiver_map.map_user_id IN ( SELECT member_id FROM {$prefix}core_members AS rm WHERE name LIKE '" . Db::i()->escape_string( $args['search'] ) . "%') )";
				}
			...
			try
			{
				$count	= Db::i()->select( 'COUNT(*)', 'core_message_posts', $where, NULL, NULL, 'msg_topic_id' )
					->join(
						'core_message_topics',
		...
		if( $iterator )
		{
			return new ActiveRecordIterator( $iterator, 'IPS\core\Messenger\Conversation' );
		}
